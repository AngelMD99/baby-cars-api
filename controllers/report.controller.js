...
const statusReport = async function (req, reply){    
    ...
    for (let element of statuses) {
        ...
        let headers=[]; //created array for column names
        let wscols=[]; //array to store the width of the columns   
        let newSheet = element.Etiqueta ? element.Etiqueta : "Carrito "+i
        wb.SheetNames.push(newSheet);
        //addig the titles rows
        var ws_data = [['Sucursal','','','','','','','','','','','','']]
        var ws = xlsx.utils.aoa_to_sheet(ws_data);       
        ...
        wb.Sheets[newSheet]["B1"].v= element['Sucursal código']!=null ? element['Sucursal código']+"-"+element['Sucursal nombre'] : "No registrada";
        wb.Sheets[newSheet]["B1"].s ={
            font:{
                bold:false
            }
        ...
        wb.Sheets[newSheet]["B2"].v= element['Modelo']!=null ? element['Modelo'] :"";
        wb.Sheets[newSheet]["B2"].s ={
            font:{
                 bold:false
            }
        ...
        wb.Sheets[newSheet]["B4"].v= element['Color']!=null ? element['Color']:"";
        wb.Sheets[newSheet]["B4"].s ={
            font:{
                 bold:false
            }
    ...
    //     wb.Sheets[newSheet]["A2"]={};
        if(req.query.initialDate != null && req.query.lastDate !=null ){ 
            let startDate = new Date (req.query.initialDate);
            let finalDate = new Date (req.query.lastDate);            
            let filteredRecords = element.records.filter( record=>{
                return record.dateTime >= startDate && record.dateTime<=finalDate
            })

            element.records = filteredRecords;
    ...
    // }  
        ...
        //if (element.records.length>0){            
            for (let index = 0; index < element.records.length; index++) {                              
                xlsx.utils.sheet_add_aoa(wb.Sheets[newSheet], [
                    ['A', 'B','C']
                  ], {origin: -1});                     
            }
    ...
    reply.type("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    reply.header("Content-Disposition", 'attachment; filename="Estados.xlsx"');
    return reply.send(buf); 
